...
@section('category_navbar')
    <div class="col-md-12 d-flex justify-content-between align-items-center position-relative">
        <h1 class="mb-2 text-secondary" id="category_title">All Categories </h1>
        ...
        <div class="" id="categoryFilterContent">
            ...
            <a href="{{ route('MobileCategory') }}" class="">
                Mobile
            </a>
            <a href="{{ route('ComputerCategory') }}" class="">
                Computer
            </a>
            <a href="{{ route('CarCategory') }}" class="">
                Car
            </a>
            <a href="{{ route('SopportCategory') }}" class="">
                Sopport
            </a>
        </div>
    </div>
                ...
                <div class="card">
                    ...
                    <a href="{{ route('post_likes_comments_shares_views', $posts->id) }}"
                        ...
                        <div class="card-body" id="card-body">
                            ...
                            <p class="card-text">
                                {{ \Illuminate\Support\Str::limit($posts->description, 50, ' More...') }}
                            </p>
                    ...
                    <div class="card-footer bg-dark text-light d-flex flex-row justify-content-around">
                        ...
                        <div class="post-comment">
                            <a href="{{ route('postComments', $posts->id) }}"
                                ...
                                <span
                                    class="fa fa-comment {{ $posts->comments()->where('user_id', Auth::id())->exists() ? 'text-warning' : 'text-light' }}"></span>
                            </a>
                        ...
                        <div class="post-viewers">
                            <a href="{{ route('postViews', $posts->id) }}"
                                class="text-decoration-none text-secondary fs-5">
                                <span
                                    class="fa  {{ $posts->views()->where('user_id', Auth::id())->exists() ? 'fa-eye text-info' : 'fa-eye-slash text-light' }}"></span>
                            </a>
                        ...
                        <div class="post-share">
                            <a href="{{ route('postShares', $posts->id) }}"
                                class="text-decoration-none text-secondary fs-5">
                                <span
                                    class="fa fa-share {{ $posts->shares()->where('user_id', Auth::id())->exists() ? 'text-success' : 'text-light' }}"></span>
                            </a>
                            <span class="post-like-count text-secondary">{{ $posts->shares_count }}</span>
                        </div>
                    </div>
